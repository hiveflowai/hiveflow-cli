...
get_text_en() {
    ...
    case "$key" in
        ...
        "change_ai_model") echo "Change AI model" ;;
        "config_error") echo "❌ Configuration Error" ;;
        "configure_llm") echo "Configure your LLM" ;;
        "context_not_generated") echo "Context not generated" ;;
        "could_not_install") echo "⚠️ Could not install globally. Continuing with npx..." ;;
        "current_project") echo "📁 Current Project" ;;
        "currently_using_npx") echo "You are currently using npx to run asis-coder." ;;
        "debug_context_file") echo "DEBUG: Context file" ;;
        "debug_interactive_mode_started") echo "DEBUG: Interactive mode started" ;;
        "dependencies_verified") echo "Dependencies verified" ;;
        ...
        "global_install_available") echo "🚀 Global Installation Available" ;;
        "initial_configuration") echo "Initial configuration" ;;
        "initial_setup_title") echo "🔧 Asis-coder Initial Setup" ;;
        "initialize_project") echo "Initialize project" ;;
        "initialize_project_question") echo "Do you want to initialize this project with Asis-coder? (y/n)" ;;
        "install_globally_question") echo "Do you want to install asis-coder globally? (y/n)" ;;
        "installed_successfully") echo "✅ Asis-coder installed globally successfully!" ;;
        "installing_globally") echo "📦 Installing globally..." ;;
        "interactive_chat_mode") echo "Interactive chat mode" ;;
        "invalid_selection") echo "❌ Invalid selection" ;;
        "laravel_detected") echo "🔍 Laravel project automatically detected" ;;
        "missing_dependency") echo "Missing dependency" ;;
        "node_detected") echo "🔍 Node.js project automatically detected" ;;
        "now_you_can_use") echo "🎉 Now you can use short commands:" ;;
        "ok_continue_npx") echo "💡 No problem, you can keep using npx whenever you want." ;;
        "press_enter_continue") echo "Press Enter to continue with setup..." ;;
        "project_type_not_detected") echo "Project type not detected" ;;
        "rails_detected") echo "🔍 Ruby on Rails project automatically detected" ;;
        "react_detected") echo "🔍 React project automatically detected" ;;
        "regenerate_context") echo "Regenerate project context" ;;
        "setup_completed") echo "🎉 Setup completed successfully!" ;;
        "spring_detected") echo "🔍 Spring Boot project automatically detected" ;;
        "test_error") echo "❌ Test error" ;;
        "type_detected") echo "Type detected" ;;
        "unknown_error") echo "Unknown error" ;;
        "useful_commands") echo "💡 Useful commands to get started:" ;;
        "validating_api_config") echo "🔍 Validating API configuration..." ;;
        "vue_detected") echo "🔍 Vue.js project automatically detected" ;;
        "with_global_install") echo "With global installation you can use:" ;;
        "would_you_like_global") echo "Would you like to install it globally to use shorter commands?" ;;
        *) echo "$key" ;;
    esac
}
