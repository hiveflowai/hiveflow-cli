...
Hiveflow is **three layers on top of a single binary**. Knowing which one to use
...
| **Tickets → PR / Workers** | Support pipeline: triage, fix in a worktree, PR, metrics · plus general workers over any kanban board | Bugs reported by users; the agent turns them into reviewable PRs. Workers automate any board with a playbook |
...
- **"Let it work on its own while I sleep"** → `/tickets cron on` (bugs) or `/worker cron on` (any board).

The layers share state: the API chat (`/llm`) also feeds the native agent; the
agent keeps its state in `~/.config/coder-cli` (permissions, sessions, MCP);
...
`hiveflow loop|deploy|review|eval|intake|remote ...`.
...
**`~/.config/coder-cli/`** — the native agent state:
