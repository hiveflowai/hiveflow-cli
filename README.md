...
3. **Swarm + autonomous development** — agents distributed across multiple devices, Ralph loops over PRDs, live dashboard.
4. **Tickets → PR pipeline** — the bug agent: reads support tickets, triages them, implements in isolated worktrees, opens PRs and waits for your approval. With metrics, evals and a CI gate.
...
| [HARNESS.md](HARNESS.md) | System design: the components, principles and state |
| [LOOPS.md](LOOPS.md) | Agentic loops: the research and why each brake exists |
| [REMOTE.md](REMOTE.md) | Running tickets on nodes (this PC or another machine) |
| [PLAN.md](PLAN.md) | Modernization roadmap to the 2026 standard (what's there, what's coming) |
| [PRUEBAS.md](PRUEBAS.md) | Testing guide: full feature inventory and how to test each one |
...
```bash
git clone https://github.com/johnolven/hiveflow-cli.git
...
### 🐝 Distributed swarm (agents across devices)
| | |
|---|---|
| `/swarm` | Status · `/swarm help` for everything (wizard, devices, worktrees, start/stop/logs/attach) |
| `/agents` | Tool per agent: `list` · `choose <proj> <agent>` (menu) · `set <proj> <agent> <tool\|auto>` |
| `/dashboard` | Live swarm dashboard |

### 🧠 Autonomous development
| | |
|---|---|
| `/prd` | Generate PRDs (bootstrap/feature/merger per project type) |
| `/ralph` | Autonomous loops: `start · status · logs · stop` |
...
└── lib/engine/          # swarm engine (vendored from asis-coder):
                         # devices, worktrees, agents, ralph, prd, dashboard
```

Both engines share state with asis-coder (`~/.config/coder-cli`): permissions, sessions, hooks, mcp.json and the existing swarm work with no migration. Overridable with `HIVEFLOW_ENGINE_DIR`. Hiveflow's own config lives in `~/.config/hiveflow/config.json`.
...
`bash`, `jq`, `curl` · The AI CLIs install from the REPL (`/install all`) · The native agent only needs an API key (`/llm`) · Multi-device swarm: `bash 4+`, `ssh`, `tmux`, `redis` (guided by `/swarm wizard`).
