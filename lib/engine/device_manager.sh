...
swarm_device_help() {
    ...
    cat <<EOF
${SWARM_C_BOLD}/swarm device${SWARM_C_RESET}  -  gestión de dispositivos
...
EOF
    ...
    cat <<EOF
${SWARM_C_BOLD}/swarm device${SWARM_C_RESET}  -  device management

  add <ip> --name <n> [--type rpi|agx|other] [--user <u>] [--port <p>]
  list
  show <name>
  remove <name>
  test <name>                Test connectivity and detect tools
  setup <name>               Install dependencies (claude-cli, tmux, git)
EOF
    fi
}
...
swarm_device_add() {
    ...
    if [ -z "$ip" ] || [ -z "$name" ]; then
        swarm_error "$(hf_t "Usage: /swarm device add <ip> --name <name> [--type <type>] [--user <user>] [--port <port>]" "Uso: /swarm device add <ip> --name <nombre> [--type <tipo>] [--user <usuario>] [--port <puerto>]")"
        return 1
    fi
    if swarm_device_exists "$name"; then
        swarm_error "$(hf_t "A device named '$name' already exists. Use 'remove' first or pick another name." "Ya existe un device con nombre '$name'. Usa 'remove' primero o escoge otro nombre.")"
        return 1
    fi
    local tmp
    tmp="$(mktemp)"
    ...
    swarm_ok "$(hf_t "Device '$name' added ($user@$ip:$port, type=$type)." "Device '$name' agregado ($user@$ip:$port, tipo=$type).")"
    swarm_info "$(hf_t "To test it: /swarm device test $name" "Para probar: /swarm device test $name")"
}
...
swarm_device_list() {
    ...
    if [ "$count" = "0" ]; then
        swarm_info "$(hf_t "No devices registered. Use: /swarm device add <ip> --name <n>" "No hay devices registrados. Usa: /swarm device add <ip> --name <n>")"
        return 0
    fi
...
swarm_device_show() {
    ...
    if [ -z "$name" ] || ! swarm_device_exists "$name"; then
        swarm_error "$(hf_t "Device '$name' not found." "Device '$name' no encontrado.")"
        return 1
    fi
...
swarm_device_remove() {
    ...
    jq --arg n "$name" '.devices |= map(select(.name != $n))' \
        "$SWARM_DEVICES_FILE" > "$tmp" && mv "$tmp" "$SWARM_DEVICES_FILE"
    swarm_ok "$(hf_t "Device '$name' removed." "Device '$name' eliminado.")"
}
...
swarm_device_test() {
    ...
    swarm_info "$(hf_t "Testing connectivity to $user@$ip:$port ..." "Probando conectividad con $user@$ip:$port ...")"
    if ! ping -c1 -W2 "$ip" >/dev/null 2>&1; then
        swarm_error "$(hf_t "Ping to $ip failed" "Ping falló hacia $ip")"
    ...
    local rc=$?
    if [ $rc -ne 0 ]; then
        swarm_error "$(hf_t "SSH failed. Set up your key with: ssh-copy-id -p $port ${user}@${ip}" "SSH falló. Configura tu llave con: ssh-copy-id -p $port ${user}@${ip}")"
        echo "$ssh_out"
    ...
    if [ "$has_claude" -gt 0 ] && [ "$has_tmux" -gt 0 ] && [ "$has_git" -gt 0 ]; then
        ...
        swarm_ok "$(hf_t "Device '$name' ready for the swarm." "Device '$name' listo para el swarm.")"
    else
        swarm_device_set_status "$name" "online"
        swarm_warn "$(hf_t "Missing dependencies. Run: /swarm device setup $name" "Faltan dependencias. Ejecuta: /swarm device setup $name")"
    fi
}
...
swarm_device_setup() {
    ...
    swarm_info "$(hf_t "Installing dependencies on '$name' (requires sudo on the device)..." "Instalando dependencias en '$name' (requiere sudo en el device)...")"
...
REMOTE
    local rc=$?
    if [ $rc -eq 0 ]; then
        swarm_ok "$(hf_t "Setup completed on '$name'." "Setup completado en '$name'.")"
        swarm_device_test "$name"
    else
        swarm_error "$(hf_t "Setup failed. Check credentials/sudo on the device." "Setup falló. Revisa credenciales/sudo en el device.")"
    fi
}
...
swarm_device_cmd() {
    ...
    case "$sub" in
        ...
        *) swarm_error "$(hf_t "Unknown subcommand: $sub" "Subcomando desconocido: $sub")"; swarm_device_help; return 1 ;;
    esac
}
