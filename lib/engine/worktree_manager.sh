...
swarm_worktree_help() {
    ...
    cat <<EOF
${SWARM_C_BOLD}/swarm worktree${SWARM_C_RESET}  -  gestión de git worktrees
...
EOF
    ...
    cat <<EOF
${SWARM_C_BOLD}/swarm worktree${SWARM_C_RESET}  -  git worktree management

  create <project> <agent> <branch>   Create a worktree for an agent
  list <project>                      List the project's worktrees
  remove <project> <agent>            Remove an agent's worktree
EOF
    fi
}
...
swarm_wt_run_on_device() {
    ...
    if [ -z "$ip" ]; then
        swarm_error "$(hf_t "Device '$device' does not exist." "Device '$device' no existe.")"
        return 1
    fi
    local my_ip
...
swarm_wt_create() {
    ...
    if [ -z "$project" ] || [ -z "$agent" ] || [ -z "$branch" ]; then
        swarm_error "$(hf_t "Usage: /swarm worktree create <project> <agent> <branch>" "Uso: /swarm worktree create <project> <agent> <branch>")"
        return 1
    fi
    local pfile
    pfile="$(swarm_project_file "$project")"
    if [ ! -f "$pfile" ]; then
        swarm_error "$(hf_t "Project '$project' does not exist." "Proyecto '$project' no existe.")"
        return 1
    fi
    ...
    if [ -z "$device" ] || [ "$device" = "null" ]; then
        swarm_error "$(hf_t "Agent '$agent' is not assigned to any device in project '$project'." "Agente '$agent' no asignado a ningún device en proyecto '$project'.")"
        return 1
    fi
    ...
    base="$(swarm_wt_base_dir "$project" "$device")"
    wt_path="${base}/${agent}"
    swarm_info "$(hf_t "Creating worktree on $device: $wt_path (branch=$branch)" "Creando worktree en $device: $wt_path (branch=$branch)")"
    swarm_wt_run_on_device "$device" bash -lc "
        set -e
        cd \"$base/main\"
    ...
    "
    if [ $? -eq 0 ]; then
        swarm_ok "$(hf_t "Worktree ready for agent '$agent' on device '$device'." "Worktree listo para agente '$agent' en device '$device'.")"
        ...
        swarm_error "$(hf_t "Failed to create worktree." "Fallo creando worktree.")"
        return 1
    fi
}
...
swarm_wt_remove() {
    ...
    if [ -n "$wt" ] && [ "$wt" != "null" ]; then
        ...
        "
        swarm_ok "$(hf_t "Worktree removed for agent '$agent'." "Worktree removido para agente '$agent'.")"
    fi
}
...
swarm_worktree_cmd() {
    ...
    case "$sub" in
        ...
        *) swarm_error "$(hf_t "Unknown subcommand: $sub" "Subcomando desconocido: $sub")"; swarm_worktree_help; return 1 ;;
    esac
}
