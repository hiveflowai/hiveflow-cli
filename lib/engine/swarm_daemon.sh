...
swarm_daemon_help() {
    ...
    cat <<EOF
${SWARM_C_BOLD}/swarm daemon${SWARM_C_RESET}  -  modo worker (solo en child)
...
EOF
    ...
    cat <<EOF
${SWARM_C_BOLD}/swarm daemon${SWARM_C_RESET}  -  worker mode (child only)

  start [--foreground]   Start the worker daemon
  stop                   Stop the daemon
  status                 Daemon status
  logs [--follow]        View daemon logs
EOF
    fi
}
...
swarm_daemon_require_child() {
    if [ ! -f "$SWARM_ROLE_FILE" ]; then
        swarm_error "$(hf_t "This device has no role. Run '/swarm init --role child ...'" "Este device no tiene rol. Ejecuta '/swarm init --role child ...'")"
        return 1
    fi
    if [ "$(swarm_role_get)" != "child" ]; then
        swarm_error "$(hf_t "The daemon only runs on 'child' devices." "El daemon solo corre en devices 'child'.")"
        return 1
    fi
    return 0
}
...
swarm_daemon_handle_cmd() {
    ...
    case "$type" in
        ...
        worktree_create)
            ...
            ;;
        claude_start)
            local session wt task tool
            session="$(echo "$payload" | jq -r '.session')"
            wt="$(echo "$payload"      | jq -r '.worktree')"
            task="$(echo "$payload"    | jq -r '.task')"
            tool="$(echo "$payload"    | jq -r '.tool // "claude"')"
            [ "$tool" = "null" ] && tool="claude"
            if tmux has-session -t "$session" 2>/dev/null; then
                output="session already running: $session"
            else
                local launch_cmd
                if type cli_build_cmd &>/dev/null; then
                    launch_cmd="$(cli_build_cmd "$tool" "$task" "${CLI_AUTO_FLAGS[$tool]:-}")"
                else
                    launch_cmd="claude --dangerously-skip-permissions $(printf '%q' "$task")"
                fi
                tmux new-session -d -s "$session" -c "$wt" \
                    "$launch_cmd; bash" 2>&1
                output="started session $session in $wt (tool: $tool)"
            fi
            ;;
        claude_stop)
            local session
...
swarm_daemon_start() {
    ...
    if [ -f "$SWARM_DAEMON_PID_FILE" ]; then
        ...
        if kill -0 "$pid" 2>/dev/null; then
            swarm_warn "$(hf_t "Daemon already running (pid=$pid)" "Daemon ya corriendo (pid=$pid)")"
            return 0
        fi
    fi

    if $foreground; then
        swarm_info "$(hf_t "Starting daemon in foreground..." "Iniciando daemon en foreground...")"
        swarm_daemon_loop
    else
        ...
        swarm_ok "$(hf_t "Daemon started (pid=$pid). Logs: $SWARM_DAEMON_LOG" "Daemon iniciado (pid=$pid). Logs: $SWARM_DAEMON_LOG")"
    fi
}

swarm_daemon_stop() {
    if [ ! -f "$SWARM_DAEMON_PID_FILE" ]; then
        swarm_warn "$(hf_t "No daemon running (no pid file)" "No hay daemon corriendo (no pid file)")"
        return 0
    fi
    local pid
    pid="$(cat "$SWARM_DAEMON_PID_FILE")"
    if kill "$pid" 2>/dev/null; then
        swarm_ok "$(hf_t "Daemon stopped (pid=$pid)" "Daemon detenido (pid=$pid)")"
    else
        swarm_warn "$(hf_t "Process $pid does not exist" "Proceso $pid no existe")"
    fi
...
swarm_daemon_logs() {
    [ ! -f "$SWARM_DAEMON_LOG" ] && { swarm_info "$(hf_t "No logs yet." "Sin logs aún.")"; return 0; }
...
swarm_daemon_cmd() {
    ...
    case "$sub" in
        ...
        *) swarm_error "$(hf_t "Unknown subcommand: $sub" "Subcomando desconocido: $sub")"; swarm_daemon_help; return 1 ;;
    esac
}
