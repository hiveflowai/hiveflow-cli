...
swarm_require_jq() {
    if ! command -v jq >/dev/null 2>&1; then
        echo -e "${SWARM_C_RED}Error:${SWARM_C_RESET} $(hf_t "jq is not installed." "jq no está instalado.")"
        echo "$(hf_t "Install it with: sudo apt-get install -y jq" "Instálalo con: sudo apt-get install -y jq")"
        return 1
    fi
    return 0
}
...
swarm_require_ssh() {
    if ! command -v ssh >/dev/null 2>&1; then
        echo -e "${SWARM_C_RED}Error:${SWARM_C_RESET} $(hf_t "ssh is not installed." "ssh no está instalado.")"
        return 1
    fi
    return 0
}
...
swarm_ssh_cmd() {
    ...
    [ -z "$port" ] && port=22
    if [ -z "$ip" ] || [ -z "$user" ]; then
        swarm_error "$(hf_t "Device '$name' not found or missing ip/user." "Device '$name' no encontrado o sin ip/usuario.")"
        return 1
    fi
