...
swarm_dashboard_help() {
    ...
    cat <<EOF
${SWARM_C_BOLD}/dashboard${SWARM_C_RESET}  -  monitor swarm en tiempo real
...
${SWARM_C_BOLD}USO${SWARM_C_RESET}
  /dashboard [--mode <mode>] [--refresh <seconds>]
...
${SWARM_C_BOLD}EJEMPLO${SWARM_C_RESET}
  /dashboard
  /dashboard --mode agents --refresh 1
EOF
    else
    cat <<EOF
${SWARM_C_BOLD}/dashboard${SWARM_C_RESET}  -  real-time swarm monitor

Interactive htop-style dashboard to visualize the swarm's state.

${SWARM_C_BOLD}MODES${SWARM_C_RESET}
  all     → Full view (default)
  devices → Device status only
  agents  → Active agents only
  redis   → Redis traffic only
  logs    → Live logs only

${SWARM_C_BOLD}USAGE${SWARM_C_RESET}
  /dashboard [--mode <mode>] [--refresh <seconds>]

${SWARM_C_BOLD}CONTROLS${SWARM_C_RESET}
  q       → Quit
  d       → Devices mode
  a       → Agents mode
  r       → Redis mode
  l       → Logs mode
  v       → All mode (full view)
  +/-     → Increase/decrease refresh rate
  ↑/↓     → Scroll logs
  ENTER   → View selected item's detail
...
draw_devices() {
    ...
    if [ -z "$devices_json" ] || [ "$devices_json" = "null" ]; then
        tput_color yellow
        echo "│ No devices configured. Run: /swarm device add <name> <ip>"
        tput_reset
        return
    fi
    ...
    if [ "$count" -eq 0 ]; then
        tput_color yellow
        echo "│ No devices registered. Run: /swarm device add <name> <ip>"
        tput_reset
        return
    fi
...
swarm_dashboard_cmd() {
    ...
    if [[ "$1" == --* ]] || [ -z "$1" ]; then
        ...
        case "$sub" in
            ...
            *) swarm_error "$(hf_t "Unknown subcommand: $sub" "Subcomando desconocido: $sub")"; swarm_dashboard_help; return 1 ;;
        esac
    fi
}
