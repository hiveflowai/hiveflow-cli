...
if [ "${BASH_VERSINFO[0]:-3}" -lt 4 ]; then
  ...
  for _hf_mod in swarm_common swarm_role swarm_enroll swarm_daemon swarm_bootstrap \
    ...
    if ! source "$HIVEFLOW_ROOT/lib/engine/$_hf_mod.sh"; then
      echo "$(hf_t "hiveflow: could not load engine module: $_hf_mod" "hiveflow: no se pudo cargar el módulo del motor: $_hf_mod")" >&2
      HF_ENGINE_LOADED=""
    fi
  done
  unset _hf_mod
